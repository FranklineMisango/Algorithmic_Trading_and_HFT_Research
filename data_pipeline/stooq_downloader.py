...
from config import (
    ...
    DEFAULT_STOOQ_INDICES, DEFAULT_STOOQ_COMMODITIES, DEFAULT_OUTPUT_FORMAT
)
from utils import convert_to_lean_format, create_zip_file, setup_logging, format_symbol_for_lean, DataValidator, write_raw_csv, write_parquet
...
class StooqDownloader:
    ...
    def download_stock_symbols(self, symbols: List[str], start_date: datetime = None, end_date: datetime = None,
                              market: str = '.US', interval: str = 'd', output_format: Optional[str] = None):
        """
            ...
            try:
                ...
                if df is not None and not df.empty:
                    if output_format in ['lean', 'lean_zip']:
                        ...
                        # Write raw CSV from DataFrame
                        raw_filename = f"{symbol.lower()}_equity_stooq.csv"
                        raw_path = os.path.join(self.data_path, raw_filename)
                        records = []
                        ...
                        raw_filename = f"{symbol.lower()}_equity_stooq.parquet"
                        raw_path = os.path.join(self.data_path, raw_filename)
                        # Use pandas to write parquet
                        try:
                            df.reset_index().rename(columns={'index': 'timestamp'}).to_parquet(raw_path, index=False)
    ...
    def download_forex_pairs(self, pairs: List[str], start_date: datetime, end_date: datetime, output_format: Optional[str] = None):
        """Download multiple forex pairs and save in Lean format or raw"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT
        logger.info(f"Starting download of {len(pairs)} forex pairs from Stooq (format={output_format})")
            ...
            try:
                ...
                if df is not None and not df.empty:
                    if output_format in ['lean', 'lean_zip']:
                        ...
                        raw_filename = f"{pair.replace('/','').lower()}_forex_stooq.csv"
                        raw_path = os.path.join(self.data_path, raw_filename)
                        records = []
                        for idx, row in df.iterrows():
                            records.append({
                        ...
                        raw_filename = f"{pair.replace('/','').lower()}_forex_stooq.parquet"
                        raw_path = os.path.join(self.data_path, raw_filename)
                        try:
                            df.reset_index().rename(columns={'index': 'timestamp'}).to_parquet(raw_path, index=False)
                            logger.info(f"Saved parquet ({raw_path}) for {pair}")
                        except Exception as e:
                            logger.error(f"Failed to write parquet for {pair}: {e}")
                else:
    ...
    def download_indices(self, indices: List[str], start_date: datetime, end_date: datetime, output_format: Optional[str] = None):
        """Download multiple indices and save in Lean or raw formats"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT
        logger.info(f"Starting download of {len(indices)} indices from Stooq (format={output_format})")
            ...
            try:
                ...
                if df is not None and not df.empty:
                    if output_format in ['lean', 'lean_zip']:
                        ...
                        raw_filename = f"{index.replace('^','').lower()}_index_stooq.csv"
                        raw_path = os.path.join(self.data_path, raw_filename)
                        records = []
                        for idx, row in df.iterrows():
                            records.append({
                        ...
                        write_raw_csv(records, raw_path)
                        logger.info(f"Saved raw CSV ({raw_path}) for {index}")
                    elif output_format == 'parquet':
                        raw_filename = f"{index.replace('^','').lower()}_index_stooq.parquet"
                        raw_path = os.path.join(self.data_path, raw_filename)
                        try:
                            df.reset_index().rename(columns={'index': 'timestamp'}).to_parquet(raw_path, index=False)
                            logger.info(f"Saved parquet ({raw_path}) for {index}")
                        except Exception as e:
                            logger.error(f"Failed to write parquet for {index}: {e}")
                else:
    ...
    def download_commodities(self, commodities: List[str], start_date: datetime, end_date: datetime, output_format: Optional[str] = None):
        """Download multiple commodities and save in Lean or raw formats"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT
        logger.info(f"Starting download of {len(commodities)} commodities from Stooq (format={output_format})")
            ...
            try:
                ...
                if df is not None and not df.empty:
                    if output_format in ['lean', 'lean_zip']:
                        ...
                        raw_filename = f"{commodity.replace('.','').lower()}_commodity_stooq.csv"
                        raw_path = os.path.join(self.data_path, raw_filename)
                        records = []
                        for idx, row in df.iterrows():
                            records.append({
                        ...
                        write_raw_csv(records, raw_path)
                        logger.info(f"Saved raw CSV ({raw_path}) for {commodity}")
                    elif output_format == 'parquet':
                        raw_filename = f"{commodity.replace('.','').lower()}_commodity_stooq.parquet"
                        raw_path = os.path.join(self.data_path, raw_filename)
                        try:
                            df.reset_index().rename(columns={'index': 'timestamp'}).to_parquet(raw_path, index=False)
                            logger.info(f"Saved parquet ({raw_path}) for {commodity}")
                        except Exception as e:
                            logger.error(f"Failed to write parquet for {commodity}: {e}")
                else:
    ...
    def download_crypto_symbols(self, symbols: List[str], start_date: datetime, end_date: datetime, output_format: Optional[str] = None):
        """Download multiple crypto symbols and save in Lean or raw formats"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT
        logger.info(f"Starting download of {len(symbols)} crypto symbols from Stooq (format={output_format})")
            ...
            try:
                ...
                if df is not None and not df.empty:
                    if output_format in ['lean', 'lean_zip']:
                        ...
                        raw_filename = f"{symbol.replace('USD','').lower()}_crypto_stooq.csv"
                        raw_path = os.path.join(self.data_path, raw_filename)
                        records = []
                        for idx, row in df.iterrows():
                            records.append({
                        ...
                        raw_filename = f"{symbol.replace('USD','').lower()}_crypto_stooq.parquet"
                        raw_path = os.path.join(self.data_path, raw_filename)
                        try:
                            df.reset_index().rename(columns={'index': 'timestamp'}).to_parquet(raw_path, index=False)
                            logger.info(f"Saved parquet ({raw_path}) for {symbol}")
