...
from config import DEFAULT_OUTPUT_FORMAT, RAW_MACRO_PATH
...
class FREDDownloader:
    ...
    def download_economic_series(self, series_ids: List[str], start_date: datetime, end_date: datetime, output_format: Optional[str] = None):
        """Download multiple economic data series"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT

        logger.info(f"Starting FRED download for {len(series_ids)} series (format={output_format})")
                    ...
                    if cleaned_data:
                        if output_format in ['lean', 'lean_zip']:
                            ...
                            if csv_content:
                                ...
                                logger.info(f"Saved {len(csv_content)} data points for {series_id} (lean)")
                        elif output_format in ['raw', 'csv', 'raw_csv']:
                            ensure_directory_exists(RAW_MACRO_PATH)
                            raw_path = os.path.join(RAW_MACRO_PATH, f"{series_id.lower()}_daily.csv")
                            write_raw_csv(cleaned_data, raw_path)
                            logger.info(f"Saved raw CSV ({raw_path}) for {series_id}")
                        elif output_format == 'parquet':
                            ensure_directory_exists(RAW_MACRO_PATH)
                            raw_path = os.path.join(RAW_MACRO_PATH, f"{series_id.lower()}_daily.parquet")
                            write_parquet(cleaned_data, raw_path)
                            logger.info(f"Saved parquet ({raw_path}) for {series_id}")
