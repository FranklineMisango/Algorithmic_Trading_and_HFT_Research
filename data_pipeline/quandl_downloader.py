...
from utils import (
    ...
    DataValidator, write_raw_csv, write_parquet
)
...
class QuandlDownloader:
    ...
    def download_datasets(self, dataset_codes: List[str], start_date: datetime, end_date: datetime, output_format: Optional[str] = None):
        """Download multiple datasets from Quandl"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT
        logger.info(f"Starting Quandl download for {len(dataset_codes)} datasets (format={output_format})")
            ...
            try:
                ...
                if data:
                    ...
                    if cleaned_data:
                        ...
                        is_crypto = any(keyword in dataset_code.lower() for keyword in ['crypto', 'bitcoin', 'btc', 'eth'])
                        if output_format in ['lean', 'lean_zip']:
                            ...
                            if csv_content:
                                ...
                                logger.info(f"Saved LEAN zip ({output_path}) for {dataset_code}")
                        elif output_format in ['raw', 'csv', 'raw_csv']:
                            if is_crypto:
                                ensure_directory_exists(RAW_CRYPTO_PATH)
                                raw_filename = f"{dataset_code.replace('/','_').lower()}_daily.csv"
                            ...
                            logger.info(f"Saved raw CSV ({raw_path}) for {dataset_code}")
                        elif output_format == 'parquet':
                            if is_crypto:
                                ensure_directory_exists(RAW_CRYPTO_PATH)
                                raw_filename = f"{dataset_code.replace('/','_').lower()}_daily.parquet"
                                raw_path = os.path.join(RAW_CRYPTO_PATH, raw_filename)
                            else:
                                ensure_directory_exists(RAW_STOCKS_PATH)
                                raw_filename = f"{dataset_code.replace('/','_').lower()}_daily.parquet"
                            ...
                            logger.info(f"Saved parquet ({raw_path}) for {dataset_code}")
