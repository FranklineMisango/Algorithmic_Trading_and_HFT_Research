...
from utils import convert_to_lean_format, create_zip_file, setup_logging, format_symbol_for_lean, DataValidator, write_raw_csv
import logging
try:
...
class InvestingComDownloader:
    ...
    def download_stock_symbols(self, symbols: List[str], start_date: datetime, end_date: datetime, country: str = 'United States', output_format: Optional[str] = None):
        """Download multiple stock symbols and save in Lean or raw formats"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT
        logger.info(f"Starting download of {len(symbols)} stock symbols from {country} (format={output_format})")
            ...
            try:
                ...
                if df is not None and not df.empty:
                    if output_format in ['lean', 'lean_zip']:
                        ...
                        raw_path = os.path.join(RAW_STOCKS_PATH, f"{symbol.lower()}_investing.csv")
                        records = []
                        for idx, row in df.iterrows():
                            records.append({'timestamp': idx.to_pydatetime() if hasattr(idx, 'to_pydatetime') else idx,
                        ...
                        ensure_directory_exists(RAW_STOCKS_PATH)
                        raw_path = os.path.join(RAW_STOCKS_PATH, f"{symbol.lower()}_investing.parquet")
                        try:
    ...
    def download_forex_pairs(self, pairs: List[str], start_date: datetime, end_date: datetime, output_format: Optional[str] = None):
        """Download multiple forex pairs and save in Lean or raw formats"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT
        logger.info(f"Starting download of {len(pairs)} forex pairs (format={output_format})")
            ...
            try:
                ...
                if df is not None and not df.empty:
                    if output_format in ['lean', 'lean_zip']:
                        ...
                        ensure_directory_exists(RAW_STOCKS_PATH)
                        raw_path = os.path.join(RAW_STOCKS_PATH, f"{pair.replace('/','').lower()}_forex_investing.csv")
                        records = []
                        for idx, row in df.iterrows():
                            records.append({'timestamp': idx.to_pydatetime() if hasattr(idx, 'to_pydatetime') else idx,
                                            'open': float(row['Open']), 'high': float(row['High']), 'low': float(row['Low']),
                        ...
                        ensure_directory_exists(RAW_STOCKS_PATH)
                        raw_path = os.path.join(RAW_STOCKS_PATH, f"{pair.replace('/','').lower()}_forex_investing.parquet")
                        try:
                            df.reset_index().rename(columns={'index':'timestamp'}).to_parquet(raw_path, index=False)
    ...
    def download_commodities(self, commodities: List[str], start_date: datetime, end_date: datetime, output_format: Optional[str] = None):
        ...
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT
        logger.info(f"Starting download of {len(commodities)} commodities (format={output_format})")
            ...
            try:
                ...
                if df is not None and not df.empty:
                    if output_format in ['lean', 'lean_zip']:
                        ...
                        ensure_directory_exists(RAW_STOCKS_PATH)
                        raw_path = os.path.join(RAW_STOCKS_PATH, f"{commodity.replace(' ','_').lower()}_commodity_investing.csv")
                        records = []
                        for idx, row in df.iterrows():
                            records.append({'timestamp': idx.to_pydatetime() if hasattr(idx, 'to_pydatetime') else idx,
                                            'open': float(row['Open']), 'high': float(row['High']), 'low': float(row['Low']),
                        ...
                        ensure_directory_exists(RAW_STOCKS_PATH)
                        raw_path = os.path.join(RAW_STOCKS_PATH, f"{commodity.replace(' ','_').lower()}_commodity_investing.parquet")
                        try:
                            df.reset_index().rename(columns={'index':'timestamp'}).to_parquet(raw_path, index=False)
    ...
    def download_crypto_symbols(self, symbols: List[str], start_date: datetime, end_date: datetime, output_format: Optional[str] = None):
        ...
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT
        logger.info(f"Starting download of {len(symbols)} crypto symbols (format={output_format})")
            ...
            try:
                ...
                if df is not None and not df.empty:
                    if output_format in ['lean', 'lean_zip']:
                        ...
                        raw_path = os.path.join(RAW_CRYPTO_PATH, f"{symbol.replace(' ','_').lower()}_crypto_investing.csv")
                        records = []
                        for idx, row in df.iterrows():
                            records.append({'timestamp': idx.to_pydatetime() if hasattr(idx, 'to_pydatetime') else idx,
                                            'open': float(row['Open']), 'high': float(row['High']), 'low': float(row['Low']),
                        ...
                        ensure_directory_exists(RAW_CRYPTO_PATH)
                        raw_path = os.path.join(RAW_CRYPTO_PATH, f"{symbol.replace(' ','_').lower()}_crypto_investing.parquet")
                        try:
                            df.reset_index().rename(columns={'index':'timestamp'}).to_parquet(raw_path, index=False)
                            logger.info(f"Saved parquet ({raw_path}) for {symbol}")
                        except Exception as e:
