...
from config import (
    TIINGO_API_KEY, EQUITY_DATA_PATH, CRYPTO_DATA_PATH, LEAN_TIMEZONE_EQUITY, LEAN_TIME_FORMAT, DEFAULT_OUTPUT_FORMAT, RAW_STOCKS_PATH, RAW_CRYPTO_PATH, RAW_FOREX_PATH
)
from utils import (
...
class TiingoDownloader:
    ...
    def download_stock_symbols(self, symbols: List[str], start_date: datetime, end_date: datetime, frequency: str = 'daily', output_format: Optional[str] = None):
        """Download stock data for multiple symbols"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT

        logger.info(f"Starting Tiingo stock download for {len(symbols)} symbols (format={output_format})")
                ...
                if data:
                    ...
                    if cleaned_data:
                        if output_format in ['lean', 'lean_zip']:
                            ...
                            raw_path = os.path.join(RAW_STOCKS_PATH, f"{symbol.lower()}_tiingo_{frequency}.csv")
                            ...
                            ensure_directory_exists(RAW_STOCKS_PATH)
                            raw_path = os.path.join(RAW_STOCKS_PATH, f"{symbol.lower()}_tiingo_{frequency}.parquet")
                            try:
                                write_parquet(cleaned_data, raw_path)
                                logger.info(f"Saved parquet to {raw_path}")
                            except Exception as e:
    ...
    def download_crypto_symbols(self, symbols: List[str], start_date: datetime, end_date: datetime, frequency: str = 'daily', output_format: Optional[str] = None):
        """Download crypto data for multiple symbols"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT

        logger.info(f"Starting Tiingo crypto download for {len(symbols)} symbols (format={output_format})")
                ...
                if data:
                    ...
                    if cleaned_data:
                        if output_format in ['lean', 'lean_zip']:
                            ...
                            raw_path = os.path.join(RAW_CRYPTO_PATH, f"{symbol.lower()}_tiingo_{frequency}.csv")
                            write_raw_csv(cleaned_data, raw_path)
                            logger.info(f"Saved raw CSV to {raw_path}")
                        elif output_format == 'parquet' and write_parquet is not None:
                            ensure_directory_exists(RAW_CRYPTO_PATH)
                            raw_path = os.path.join(RAW_CRYPTO_PATH, f"{symbol.lower()}_tiingo_{frequency}.parquet")
                            try:
                                write_parquet(cleaned_data, raw_path)
                                logger.info(f"Saved parquet to {raw_path}")
                            except Exception as e:
    ...
    def download_forex_pairs(self, pairs: List[str], start_date: datetime, end_date: datetime, frequency: str = 'daily', output_format: Optional[str] = None):
        """Download forex data for multiple pairs"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT

        logger.info(f"Starting Tiingo forex download for {len(pairs)} pairs (format={output_format})")
                ...
                if data:
                    ...
                    if cleaned_data:
                        if output_format in ['lean', 'lean_zip']:
                            ...
                            if csv_content:
                                write_lean_zip_file(csv_content, output_path, csv_filename)
                                logger.info(f"Saved {len(csv_content)} bars for {pair} (lean)")
                        elif output_format in ['raw', 'csv', 'raw_csv']:
                            ensure_directory_exists(RAW_FOREX_PATH)
                            raw_path = os.path.join(RAW_FOREX_PATH, f"{pair.lower().replace('/','')}_tiingo_{frequency}.csv")
                            write_raw_csv(cleaned_data, raw_path)
                            logger.info(f"Saved raw CSV to {raw_path}")
                        elif output_format == 'parquet' and write_parquet is not None:
                            ensure_directory_exists(RAW_FOREX_PATH)
                            raw_path = os.path.join(RAW_FOREX_PATH, f"{pair.lower().replace('/','')}_tiingo_{frequency}.parquet")
                            try:
                                write_parquet(cleaned_data, raw_path)
                                logger.info(f"Saved parquet to {raw_path}")
                            except Exception as e:
