...
class CoinDeskDownloader:
    ...
    def download_bitcoin_data(self, start_date: datetime, end_date: datetime, output_format: Optional[str] = None):
        """Download Bitcoin price index data"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT
        logger.info(f"Starting CoinDesk Bitcoin download (format={output_format})")
                ...
                if cleaned_data:
                    if output_format in ['lean', 'lean_zip']:
                        ...
                        if csv_content:
                            ...
                            logger.info(f"Saved {len(csv_content)} Bitcoin price points (lean)")
                    elif output_format in ['raw', 'csv', 'raw_csv']:
                        ensure_directory_exists(RAW_CRYPTO_PATH)
                        raw_path = os.path.join(RAW_CRYPTO_PATH, 'coindesk_btc_usd.csv')
                        ...
                        ensure_directory_exists(RAW_CRYPTO_PATH)
                        raw_path = os.path.join(RAW_CRYPTO_PATH, 'coindesk_btc_usd.parquet')
                        try:
