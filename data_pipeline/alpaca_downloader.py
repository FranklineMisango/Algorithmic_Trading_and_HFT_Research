...
from config import DEFAULT_OUTPUT_FORMAT, RAW_STOCKS_PATH
from utils import (
...
class AlpacaDataDownloader:
    ...
    def download_symbol_data(self, symbol: str, resolution: str, start_date: datetime, end_date: datetime, output_format: Optional[str] = None):
        """Download and save data for a single symbol"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT

        logger.info(f"Downloading {symbol} data for {resolution} resolution (format={output_format})")
                ...
                if cleaned_data:
                    if output_format in ['lean', 'lean_zip']:
                        ...
                        if all_csv_content:
                            ...
                            logger.info(f"Saved {len(all_csv_content)} bars for {symbol} {resolution} (lean)")
                        ...
                        if write_raw_csv is not None:
                            ...
                            # Fallback: write via pandas
                            pd.DataFrame(cleaned_data).to_csv(raw_path, index=False)
    ...
    def download_multiple_symbols(self, symbols: List[str], resolution: str, start_date: datetime, end_date: datetime, output_format: Optional[str] = None):
        """Download data for multiple symbols (supports output_format: lean/raw/parquet)"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT
