...
def write_raw_csv(data: List[Dict], output_path: str):
    """Write raw OHLCV list-of-dicts to CSV with ISO timestamps"""
    ensure_directory_exists(os.path.dirname(output_path))
...
def write_parquet(data: List[Dict], output_path: str):
    """Write raw OHLCV list-of-dicts to Parquet using pandas."""
    ensure_directory_exists(os.path.dirname(output_path))
    try:
        df = pd.DataFrame(data)
        if 'timestamp' in df.columns:
            df['timestamp'] = pd.to_datetime(df['timestamp'])
        # Let pandas choose engine (pyarrow/fastparquet) if available
