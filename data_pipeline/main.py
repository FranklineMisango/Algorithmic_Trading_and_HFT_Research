...
def main():
    ...
    parser.add_argument('--output-format', choices=['lean','raw','parquet','csv'], default=None,
                       help='Output format: lean (QuantConnect LEAN zip), raw (flat CSV), parquet')
    ...
    if args.source in ['alpha-vantage', 'all']:
        try:
            ...
            # Download stocks
            av_downloader.download_stock_symbols(args.av_stocks, args.resolution, args.start_date, args.end_date, output_format=args.output_format)
            
            # Download forex
            forex_pairs = [(pair[:3], pair[3:]) for pair in args.av_forex if len(pair) == 6]
            av_downloader.download_forex_pairs(forex_pairs, args.resolution, args.start_date, args.end_date, output_format=args.output_format)
            
            # Download crypto
            av_downloader.download_crypto_symbols(args.av_crypto, args.resolution, args.start_date, args.end_date, output_format=args.output_format)
    ...
    if args.source in ['yahoo', 'all']:
        try:
            ...
            # Download stocks
            yahoo_downloader.download_stock_symbols(args.equity_symbols, args.resolution, args.start_date, args.end_date, 'equity', output_format=args.output_format)
            
            # Download ETFs
            yahoo_downloader.download_stock_symbols(args.yahoo_etfs, args.resolution, args.start_date, args.end_date, 'etf', output_format=args.output_format)
            
            # Download indices
            yahoo_downloader.download_stock_symbols(args.yahoo_indices, args.resolution, args.start_date, args.end_date, 'indices', output_format=args.output_format)
            
            # Download forex
            yahoo_downloader.download_forex_pairs(args.yahoo_forex, args.resolution, args.start_date, args.end_date, output_format=args.output_format)
            
            # Download crypto
            yahoo_downloader.download_crypto_symbols(args.yahoo_crypto, args.start_date, args.end_date, args.resolution, output_format=args.output_format)
    ...
    if args.source in ['nse-india', 'all']:
        try:
            ...
            nse_downloader.download_equity_symbols(args.nse_stocks, args.start_date, args.end_date, output_format=args.output_format)
            
            # Download index data
            nse_downloader.download_index_symbols(args.nse_indices, args.start_date, args.end_date, output_format=args.output_format)
    ...
    if args.source in ['bse-india', 'all']:
        try:
            ...
            # Download equity data
            bse_downloader.download_equity_symbols(args.bse_stocks, args.start_date, args.end_date, output_format=args.output_format)
    ...
    if args.source in ['tiingo', 'all']:
        try:
            ...
            # Download stocks
            tiingo_downloader.download_stock_symbols(args.tiingo_stocks, args.start_date, args.end_date, args.resolution, output_format=args.output_format)
            
            # Download crypto
            tiingo_downloader.download_crypto_symbols(args.tiingo_crypto, args.start_date, args.end_date, args.resolution, output_format=args.output_format)
            
            # Download forex
            tiingo_downloader.download_forex_pairs(args.tiingo_forex, args.start_date, args.end_date, args.resolution, output_format=args.output_format)
    ...
    if args.source in ['fred', 'all']:
        try:
            ...
            fred_downloader.download_economic_series(args.fred_series, args.start_date, args.end_date, output_format=args.output_format)
    ...
    if args.source in ['quandl', 'all']:
        try:
            ...
            # Download datasets
            quandl_downloader.download_datasets(args.quandl_datasets, args.start_date, args.end_date, output_format=args.output_format)
    ...
    if args.source in ['coindesk', 'all']:
        try:
            ...
            coindesk_downloader.download_bitcoin_data(args.start_date, args.end_date, output_format=args.output_format)
            
            # Download current prices
            coindesk_downloader.download_current_prices(['USD', 'EUR', 'GBP'], output_format=args.output_format)
