...
from config import DEFAULT_OUTPUT_FORMAT, RAW_STOCKS_PATH, RAW_CRYPTO_PATH
...
class AlphaVantageDownloader:
    ...
    def download_stock_symbols(self, symbols: List[str], resolution: str, start_date: datetime, end_date: datetime, output_format: Optional[str] = None):
        """Download stock data for multiple symbols"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT
        logger.info(f"Starting Alpha Vantage stock download for {len(symbols)} symbols (format={output_format})")
                    ...
                    if filtered_data:
                        ...
                        if cleaned_data:
                            if output_format in ['lean', 'lean_zip']:
                                ...
                                raw_path = os.path.join(RAW_STOCKS_PATH, f"{symbol.lower()}_{resolution}.csv")
                                write_raw_csv(cleaned_data, raw_path)
                                logger.info(f"Saved raw CSV ({raw_path}) for {symbol}")
                            elif output_format == 'parquet':
                                ensure_directory_exists(RAW_STOCKS_PATH)
                                raw_path = os.path.join(RAW_STOCKS_PATH, f"{symbol.lower()}_{resolution}.parquet")
    ...
    def download_forex_pairs(self, pairs: List[tuple], resolution: str, start_date: datetime, end_date: datetime, output_format: Optional[str] = None):
        """Download forex data for multiple currency pairs"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT
        logger.info(f"Starting Alpha Vantage forex download for {len(pairs)} pairs (format={output_format})")
                    ...
                    if filtered_data:
                        ...
                        if cleaned_data:
                            ...
                            if output_format in ['lean', 'lean_zip']:
                                ...
                                if csv_content:
                                    write_lean_zip_file(csv_content, output_path, csv_filename)
                                    logger.info(f"Saved LEAN zip ({output_path}) for {pair_symbol}")
                            elif output_format in ['raw', 'csv', 'raw_csv']:
                                ensure_directory_exists(RAW_STOCKS_PATH)
                                raw_path = os.path.join(RAW_STOCKS_PATH, f"{pair_symbol.lower()}_{resolution}.csv")
                                write_raw_csv(cleaned_data, raw_path)
                                logger.info(f"Saved raw CSV ({raw_path}) for {pair_symbol}")
                            elif output_format == 'parquet':
                                ensure_directory_exists(RAW_STOCKS_PATH)
                                raw_path = os.path.join(RAW_STOCKS_PATH, f"{pair_symbol.lower()}_{resolution}.parquet")
                                write_parquet(cleaned_data, raw_path)
                                logger.info(f"Saved parquet ({raw_path}) for {pair_symbol}")
                
            except Exception as e:
                logger.error(f"Error downloading {from_symbol}/{to_symbol}: {str(e)}")
                continue
    
    def download_crypto_symbols(self, symbols: List[str], start_date: datetime, end_date: datetime, resolution: str = 'daily', output_format: Optional[str] = None):
        """Download crypto data for multiple symbols (orchestrator-compatible method)"""
        if output_format is None:
            output_format = DEFAULT_OUTPUT_FORMAT
        logger.info(f"Starting Alpha Vantage crypto download for {len(symbols)} symbols (format={output_format})")
                    ...
                    if filtered_data:
                        ...
                        if cleaned_data:
                            ...
                            if output_format in ['lean', 'lean_zip']:
                                ...
                                raw_path = os.path.join(RAW_CRYPTO_PATH, f"{symbol.lower()}_{resolution}.csv")
                                write_raw_csv(cleaned_data, raw_path)
                                logger.info(f"Saved raw CSV ({raw_path}) for {symbol}")
                            elif output_format == 'parquet':
                                ensure_directory_exists(RAW_CRYPTO_PATH)
                                raw_path = os.path.join(RAW_CRYPTO_PATH, f"{symbol.lower()}_{resolution}.parquet")
                                write_parquet(cleaned_data, raw_path)
                                logger.info(f"Saved parquet ({raw_path}) for {symbol}")
