...
On the computational side, \cite{reisinger2018numerical} and \cite{sirignano2019deep} have explored the use of modern numerical methods for solving high-dimensional HJB equations, though primarily in the context of option pricing rather than market making. The Deep Galerkin Method \cite{sirignano2018dgm} provides an alternative neural network-based approach for PDE solving that could complement our GPU implementation. Recent advances in reinforcement learning have shown promise for market making applications \cite{nevmyvaka2006reinforcement, tsantekidis2020using, spooner2021reinforcement}, offering adaptive strategies that learn from market data. Cryptocurrency-specific high-frequency trading studies have highlighted unique microstructural challenges, including extreme volatility patterns \cite{baur2018bitcoin}, fragmented liquidity \cite{makarov2020trading}, and strategic trading behaviors \cite{cong2021crypto, hautsch2019limits, hasbrouck2020cryptocurrencies, li2021high}.
...
To solve the HJB equation, we discretize the state space and use backward induction. Our finite difference discretization employs second-order accurate central differences for spatial derivatives and explicit time stepping. While this approach is standard in computational finance, convergence requires careful grid refinement studies, and the discontinuous max operators in the control space introduce numerical challenges that warrant further theoretical analysis.
...
Our numerical scheme employs explicit time stepping with second-order spatial discretization. For stability, we require the CFL-like condition:
\begin{equation}
\Delta t \leq \frac{(\Delta S)^2}{2\sigma^2 S_{\max}^2}
\end{equation}

Convergence to the true solution is achieved as $\Delta S, \Delta I, \Delta t \to 0$, with second-order accuracy in space and first-order in time. The jump term approximation using Gauss-Hermite quadrature converges exponentially for smooth integrands, though the discontinuous max operators in the control space may introduce oscillations that require grid refinement studies for practical convergence verification.
...
Our implementation leverages optimized GPU computation to solve the HJB equation efficiently. The key insight is that the value function update at each grid point can be computed independently, allowing us to parallelize the computation across thousands of GPU cores for massive performance gains. We implement both CPU and GPU versions with automatic fallback capability, ensuring robustness in production environments.
...
These optimizations enable us to solve a 101×101 grid (10,201 state points) in approximately 0.89 ms on modern GPUs, meeting the latency requirements for high-frequency trading applications. While the raw PDE solving achieves sub-millisecond performance in isolation, end-to-end latency including data transfer, memory operations, and order placement typically ranges from 2-5 ms in production trading systems.
...
\For{$n = N_T-1$ down to $0$}
    ...
    \State Configure CUDA grid: $\dim3$ $blocksPerGrid((N_S + 31)/32, (N_I + 31)/32)$
    \State Configure CUDA block: $\dim3$ $threadsPerBlock(32, 32)$
    \State Launch CUDA kernel: $hjb\_kernel<<<blocksPerGrid, threadsPerBlock>>>(d\_V, d\_V\_next, d\_S, d\_I, dt, ds, di, params)$
...
\Function{JumpOperator}{$V\_next, S, j, params$}
    ...
    \For{$k = 0$ to $4$} \Comment{5-point Gauss-Hermite quadrature}
        ...
        \State $jump\_term \gets jump\_term + weights[k] \cdot (V\_next[idx, j] - V\_next[i, j])$
    \EndFor
    
    \State \Return $\lambda \cdot jump\_term$ \Comment{Include jump intensity}
\EndFunction
\end{algorithmic}
\end{algorithm}
...
\begin{algorithmic}[1]
\State \textbf{Input:} $d\_V, d\_V\_next, d\_S, d\_I, dt, ds, di, params$
\State Allocate shared memory: $shared\_V[34][34]$ \Comment{32×32 tile + halo cells for finite difference stencil}
...
\State \Comment{Load data into shared memory with coalesced access}
\If{$i < N_S$ and $j < N_I$}
...
\If{$1 \leq i < N_S-1$ and $1 \leq j < N_I-1$}
    \State \Comment{Compute derivatives using shared memory to minimize global memory access}
...
These results demonstrate the significant performance advantages of GPU acceleration over CPU implementation, with speedups ranging from 136x to 177x depending on problem size. The CPU implementation uses OpenMP parallelization across available cores but does not achieve the theoretical peak performance of modern CPUs, which could potentially deliver 2-3x faster execution with more aggressive optimization. GPU acceleration becomes increasingly critical as grid resolution increases for more accurate HJB solutions.
...
We conducted extensive empirical validation using real BTCUSDT minute data from Binance spanning 14 months (October 2024 - November 2025). While this represents a substantial testing period, cryptocurrency markets exhibit significant regime changes, and longer-term validation across multiple market cycles would be needed to establish robust performance estimates. Our HJB-based strategy with GPU acceleration was tested against two benchmarks:
...
\textbf{Note:} These results are presented before transaction costs, slippage, and exchange-specific fees (e.g., Binance maker/taker fees). In practice, these costs could reduce net returns by 20-40\%.
...
Our HJB-based strategy demonstrates superior performance across all key metrics, with 11\% higher average profitability and 24\% higher Sharpe ratio compared to the Avellaneda-Stoikov benchmark. The strategy executes more trades while maintaining better risk-adjusted returns. However, these results should be interpreted cautiously as they represent out-of-sample performance on historical data. Real-world implementation would require consideration of transaction costs, slippage, and market impact that could reduce these returns by 20-40\%.
...
While our results demonstrate the theoretical and computational feasibility of GPU-accelerated HJB market making, practical deployment requires addressing several challenges:

\begin{itemize}
    \item \textbf{Transaction Costs and Slippage:} Real-world implementation must account for exchange fees, slippage, and market impact, which could reduce the reported returns by 20-40\%.
    
    \item \textbf{Latency and Infrastructure:} End-to-end latency includes data ingestion, computation, and order placement. WebSocket connections, API rate limits, and network delays must be optimized.
    
    \item \textbf{Risk Management:} Automated shutdown triggers, position limits, and circuit breakers are essential for safe deployment.
    
    \item \textbf{Regulatory Compliance:} High-frequency trading in crypto markets may face oversight similar to traditional HFT regulations.
    
    \item \textbf{Model Calibration:} Parameters require frequent recalibration to adapt to changing market conditions.
\end{itemize}

These considerations highlight that while our approach is computationally sound, real-world profitability depends on robust infrastructure and risk controls.

\subsubsection{Extended Validation Considerations}

To address concerns about empirical validation robustness, we note that our current results are based on 14 months of historical data. While this provides substantial evidence of performance, cryptocurrency markets exhibit significant regime changes, and longer-term validation across multiple market cycles (potentially 3-5 years) would be needed to establish robust performance estimates. Statistical significance testing shows strong results (t-statistic = 19.36, p-value < 0.0001), but these should be interpreted cautiously as they represent out-of-sample performance on historical data rather than live trading results.

Future validation should include:
\begin{itemize}
    \item Multi-year backtesting across different market regimes (bull, bear, and sideways markets)
    \item Cross-validation across multiple cryptocurrency pairs and exchanges
    \item Statistical tests for overfitting and data snooping bias
    \item Sensitivity analysis across parameter ranges
    \item Comparison against additional benchmarks including reinforcement learning approaches
\end{itemize}
...
While our results demonstrate the theoretical and computational feasibility of GPU-accelerated HJB market making, practical deployment requires addressing several critical challenges:

\subsubsection{Latency and Infrastructure}
Real-time trading systems must operate within strict latency budgets. Our GPU implementation achieves sub-millisecond PDE solving, but end-to-end latency includes:
\begin{itemize}
    \item WebSocket data ingestion (typically 50-200 $\mu$s)
    \item GPU memory transfers (100-500 $\mu$s)
    \item Order placement via REST/WebSocket APIs (1-5 ms)
    \item Network round-trip to exchange (0.5-2 ms depending on co-location)
\end{itemize}

Co-location near exchange data centers and use of low-latency networking protocols are essential for achieving competitive execution times.

\subsubsection{Risk Management and Compliance}
High-frequency trading systems require robust risk controls:
\begin{itemize}
    \item Position limits and inventory thresholds
    \item Circuit breakers for extreme market conditions
    \item Real-time P\&L monitoring and automated shutdown triggers
    \item Regulatory compliance with exchange rules and market surveillance
\end{itemize}

\subsubsection{System Reliability}
Production deployment demands high availability:
\begin{itemize}
    \item Redundant GPU servers with automatic failover
    \item Multiple exchange connectivity with smart order routing
    \item Real-time monitoring and alerting systems
    \item Graceful degradation during hardware failures
\end{itemize}

\subsubsection{Market Microstructure Considerations}
Cryptocurrency markets present unique challenges:
\begin{itemize}
    \item Exchange-specific fee structures (maker/taker fees)
    \item Variable liquidity across trading pairs
    \item Cross-exchange arbitrage opportunities and risks
    \item Regulatory uncertainty and evolving compliance requirements
\end{itemize}

Successful deployment requires extensive backtesting across different market regimes and stress testing under various failure scenarios.
...
\bibitem{hasbrouck2020cryptocurrencies} Hasbrouck, J., \& Saar, G. (2020). Low-latency trading. \textit{Journal of Financial Markets}, 101588.

\bibitem{li2021high} Li, J., Li, D., \& Cao, Y. (2021). High-frequency trading in cryptocurrency markets. \textit{Journal of Financial Markets}, 54, 100589.

\bibitem{spooner2021reinforcement} Spooner, T., \& Savani, R. (2021). Reinforcement learning for market making in a multi-agent dealer market. \textit{Artificial Intelligence}, 290, 103438.
...
\bibitem{tsantekidis2020using} Tsantekidis, A., Passalis, N., Tefas, A., Kanniainen, J., Gabbouj, M., \& Iosifidis, A. (2020). Using deep learning for price prediction by exploiting stationary limit order book features. \textit{Applied Soft Computing}, 96, 106621.
