...
\begin{itemize}
    ...
    \item \textbf{CPU/GPU Acceleration as an alternate compute : }We implement optimized CPU and GPU solution methods capable of solving the high-dimensional HJB equation in sub-second time, making real-time deployment feasible even in high-frequency scenarios.
...
\begin{algorithm}
\caption{HJB Value Function Iteration with CPU Acceleration}
\begin{algorithmic}[1]
...
\For{$n = N_T-1$ down to $0$}
    \State Copy parameters including $p^b_t, p^a_t$ to memory
    \State Launch parallel CPU computation with Numba JIT
    \State Update value function for all grid points
    \State Synchronize and copy results
\EndFor
...
\begin{algorithm}
\caption{CPU Kernel for HJB Equation with Jump Diffusion}
\begin{algorithmic}[1]
\State \textbf{Input:} $V, V\_next, S\_grid, I\_grid, dt, ds, di, params$
\For{$i = 1$ to $N_S-1$} \Comment{Interior price points}
    \For{$j = 0$ to $N_I-1$} \Comment{All inventory levels}
        \If{$j = 0$ or $j = N_I-1$}
            \State $V[i,j] \gets -10^{20}$ \Comment{Enforce boundary conditions}
            \State \textbf{continue}
        \EndIf
        
        \State Extract current state: $S \gets S\_grid[i]$, $I \gets I\_grid[j]$
        ...
        \State Compute jump term: $jump\_term \gets JumpOperator(V\_next, S, j, params)$
...
\begin{itemize}
    ...
    \item \textbf{Storage:} SSD storage for data caching
...
We benchmarked our CPU implementation on a range of grid sizes:
...
\begin{axis}[
    ...
    title={HJB Solver Performance: CPU Implementation},
    legend pos=north west,
    grid=major,
    xmode=log,
    log basis x={2},
...
\legend{CPU (Intel Core i7-4790K)}
\end{axis}
\end{tikzpicture}
\caption{Performance of the CPU implementation of the HJB solver with jump diffusion. The implementation achieves sub-second performance for typical grid sizes, suitable for high-frequency trading applications.}
...
\begin{tabular}{|c|c|}
\hline
Grid Size & CPU (Intel Core i7-4790K) \\
\hline
51×51 & 32.7 ms \\
101×101 & 142.3 ms \\
201×201 & 612.5 ms \\
...
These results demonstrate that our CPU implementation achieves sub-second performance for typical grid sizes, making it suitable for high-frequency trading applications with proper optimization.
...
\begin{itemize}
    ...
    \item Optimized CPU and GPU solution methods enabling real-time deployment
    \item A comprehensive validation framework demonstrating improved risk-adjusted returns
\end{itemize}
